...
void CDXFHatchNonPolylineBoundary::AddArc(const CDXFGePoint2D &centerPoint, double dStartAngle, double dEndAngle,	double dRadius, int32_t nCounterclockwise)
{
...
void CDXFHatchNonPolylineBoundary::AddEllipse(const CDXFGePoint2D &centerPoint, const CDXFGePoint2D &endPoint, double dMinorAxisLength, double dStartAngle, 
...
{
	size_t nNumEdges = m_pBand->size();
	ASSERT(nEdgeIndex >= 0 && nEdgeIndex < nNumEdges);
