...
public:
	...
	CDXFPolyline(const std::vector<CDXFGeBulgePoint3D> &vertices, int32_t nCount, bool bClosed)
        : CDXFEntity("LWPOLYLINE"), m_bClosed(bClosed), m_vertices(vertices) { }
	CDXFPolyline(const CDXFPolyline &polyline)
        : CDXFEntity(polyline, "LWPOLYLINE"), m_eLineWeight(polyline.m_eLineWeight), 
...
public:
	...
	void AddVertex(double x, double y, double z, double bulge) {
	    ...
	    vertex.m_dBulge = bulge;
	    vertex.m_point.SetValues(x, y, z);
	    AddVertex(vertex);
    }
