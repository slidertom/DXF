...
{    
    ...
    if ((int32_t)ulHandle > m_vars.m_nHandSeed) {
...
namespace internal
{
    ...
    inline int32_t GetBlockPos(const char *str, int32_t nStart) {
        const char *sBlockRecord = "acdbblocktablerecord";
        const char *match = ::strstr(str + nStart, sBlockRecord);
        return (nullptr == match) ? -1 : (int32_t)(match - str);
    }

    inline int32_t GetLineEndPosition(const char *str, int32_t nStart) {
        const int32_t nLineEnd = 10;
        const char *match = ::strchr(str + nStart, nLineEnd);
        return (nullptr == match) ? -1 : (int32_t)(match - str);
    }

    inline bool MoveToNextToken(const char *str, int32_t nPos, int32_t& nNewPos) {
    ...
    inline char *GetDXFBlock1(const char *str, int32_t nPosBegin, int32_t nPosEnd) {
        const int32_t nSize = (nPosEnd - nPosBegin);
...
CDXFObjectID CDXFDatabase::GetTextStyleIdByName(const char *sTextStyleName) const
{
    CDXFObjectID textStyleID;
    const CDXFTextStyleTable *pTextStyleTable = GetTextStyleTable();
    pTextStyleTable->GetTableRecordId(sTextStyleName, textStyleID);
    return textStyleID;
}

CDXFObjectID CDXFDatabase::GetLayerIdByName(const char *sLayerName) const
{
    CDXFObjectID layerID;
    const CDXFLayerTable *pLayerTable = GetLayerTable();
    pLayerTable->GetTableRecordId(sLayerName, layerID);
    return layerID;
}

CDXFObjectID CDXFDatabase::GetDimStyleIdByName(const char *sDimStyleName) const
{
    CDXFObjectID dimStyleID;
    const CDXFDimStyleTable *pDimStyleTable = GetDimStyleTable();
    pDimStyleTable->GetTableRecordId(sDimStyleName, dimStyleID);
    return dimStyleID;
}

CDXFObjectID CDXFDatabase::GetTextStyleIdByNameEx(const char *sTextStyleName) const
{
    CDXFObjectID textStyleID = GetTextStyleIdByName(sTextStyleName);
    if ( textStyleID.IsNull() ) {
        return GetTextStyleIdByName("STANDARD");
    }
    return textStyleID;
}

CDXFObjectID CDXFDatabase::GetLayerIdByNameEx(const char *sLayerName) const
{
    CDXFObjectID layerID = GetLayerIdByName(sLayerName);
    if ( layerID.IsNull() ) {
        return GetLayerIdByName("0");
    }
    return layerID;
}

CDXFObjectID CDXFDatabase::GetDimStyleIdByNameEx(const char *sDimStyleName) const
{
    CDXFObjectID dimStyleID = GetDimStyleIdByName(sDimStyleName);
    if  (dimStyleID.IsNull() ) {
        return GetDimStyleIdByName("STANDARD");
    }
    return dimStyleID;
}